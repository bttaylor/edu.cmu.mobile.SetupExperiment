...
//		soundOn = false;
	 ...
	 *            screen height
	 * */
	public void initCoords(int width, int height, int level) {
		...
		//Ball always drops straight down
		velocityX = 0;
	...
	public int setVelocity() {
		...
		if (this.getBounds().top <= 0) {
			...
			try {
				...
				initCoords(SCREEN_WIDTH, SCREEN_HEIGHT, gameLevel); // reset ball
	...
	public int checkBlocksCollision(ArrayList<Block> blocks) {
		...
		Log.v("checkBlocksCollision()", "blockListLength: " + blockListLength);
	...
	public void close() {
		if (soundOn) {
			Log.i("Ball","releases the soundPool");
			soundPool.release();
			soundPool = null;
		}
	}
